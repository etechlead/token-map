...
public sealed class JavaScriptSyntaxAnalyzerTests
{
    private readonly JavaScriptSyntaxAnalyzer _analyzer = new();
    ...
    public async Task AnalyzeAsync_CollectsCallablesAndComplexity()
    {
        const string sourceText = """
            ...
            function top(x, items) {
              const fn = y => y > 0 ? y : 0;
              const anon = function(z) { return z && x ? z : x; };
    ...
    public async Task AnalyzeAsync_ReportsRecoveredForBrokenSyntax()
    {
        ...
        var summary = await _analyzer.AnalyzeAsync("broken.js", sourceText, CancellationToken.None);
